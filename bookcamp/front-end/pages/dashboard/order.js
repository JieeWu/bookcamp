...
export default function Order() {
    ...
    useEffect(() => {
        const dataTest = async () => {
            try {
                const res = await axios.get('http://3.113.3.149:3002/order')
                ...
                const getThatData = res.data.bookorder;
                const newOrder = getThatData.map((v) => {
                    return { ...v, checkThisBox: Boolean }; //加入欄位
                });
                setOrder(newOrder);
    ...
    return (
        <>
            <div className="row">
                <OrderStatus status={status} order={order} setOrderStatus={setOrderStatus} orderStatus={orderStatus} />
                <OrderTotal setOrder={setOrder} total={total} setTotal={setTotal} />
                <OrderDate setOrder={setOrder} date={date} setDate={setDate} />
                <OrderKeyword setOrder={setOrder} keyword={keyword} setkeyword={setkeyword} status={status} delivery={delivery} receipt={receipt} pay={pay} />
            </div>
            <div className="order-th-group">
                <div className='order-th-checked'>
                    <span>更改狀態</span>
                    <input className="ms-2" type="checkbox" id="checkAll" onChange={handleSelectAll} />
                </div>
            ...
            <OrderList toggleCheck={toggleCheck} coupon={coupon} delivery={delivery} order={order} date={date} status={status} pay={pay} receipt={receipt} />
