...
export default function CollectButton2({ oldBookId }) {
 ...
 const [isUnCollecting, setIsUnCollecting] = useState(false)
    ...
    const checkCollectStatus = async () => {
      try {
        const response = await axios.get(
          `http://3.113.3.149:3002/searchcollect`,
          {
  ...
  const handleCollect = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://3.113.3.149:3002/collect/oldcollect',
        { book_id: oldBookId },
        {
          headers,
          withCredentials: true,
  ...
  return (
    <button
      ...
      ${buttonClass} ${buttonClass1} ${buttonClass2}  `}
      onClick={collect ? handleUnCollect : handleCollect}
    >
