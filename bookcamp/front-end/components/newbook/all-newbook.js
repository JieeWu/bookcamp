...
const handleAlert = (e) => {
  ...
  const fetchSortedBooks = async (order ='', page = currentPage) => {
    try {
      const res = await axios.get(`http://3.113.3.149:3002/price/books`, {
    ...
    const fetchBooksByDate = async (page = currentPage) => {
      try {
        const res = await axios.get("http://3.113.3.149:3002/bookdate", {
          params: { page: page },
        });
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      ...
      if (query && router.isReady) {
        endpoint = `http://3.113.3.149:3002/hotsearch?page=${currentPage}`;
      } else if (b_genre_id) {
        endpoint = `http://3.113.3.149:3002/obgenre/${b_genre_id}?page=${currentPage}`;
      } else if (book_language) {
        endpoint = `http://3.113.3.149:3002/obgenre/language/${book_language}?page=${currentPage}`;
      } else {
        endpoint = `http://3.113.3.149:3002/perpage?page=${currentPage}`;
      }
  ...
  const handleSearch = async (searchTerm) => {
    ...
    try {
      const response = await axios.get('http://3.113.3.149:3002/hotsearch', {
                    ...
                    >
                      <Link href={'/newbook/' + v.book_id}>
                        <img
                          ...
                          src={`http://3.113.3.149:3002/public/img/oldbookimgs/${v.book_img_id}`}
                        />
                      </Link>
