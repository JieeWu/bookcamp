...
/**
 * execute sql with db(promise wrapper), log the sql and error by default
 * @param {string} sql
 ...
 * @returns
 */
const findOneById = (table, client_id) => {
  return findOne(table, { client_id });
};
 ...
 * @returns {object}
 */
const updateById = async (table, obj, client_id) => {
  return update(table, obj, { client_id });
};
...
const removeById = async (table, client_id) => {
  return remove(table, { client_id });
};
