...
router.get('/logout', async function (req, res, next) {
  try {
    ...
    const user = await findOne('client', {
    ...
    console.error('Error line logout:', error)
  ...
  LineLogin.callback(
    ...
    async (req, res, next, token_response) => {
      try {
        ...
        const isFound = await count('client', {
        ...
        if (isFound) {
          ...
          const newUser = {
            client_name: token_response.id_token.name,
            email: '',
            line_uid: token_response.id_token.sub,
        ...
        console.error('Error line login callback:', error)
        return res.status(500).json({ message: 'internal server error' })
      }
    },
