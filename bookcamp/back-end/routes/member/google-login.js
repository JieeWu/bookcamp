...
router.post('/jwt', async function (req, res, next) {
  try {
    ...
    console.log('user1', JSON.stringify(providerData))
    ...
    const isFound = await count('client', { google_uid: providerData.uid })

    if (isFound) {
      // 有存在 -> 執行登入工作
      const user = await findOne('client', { google_uid: providerData.uid })
      ...
      const newUser = {
        client_name: providerData.displayName,
        email: providerData.email,
        google_uid: providerData.uid,
        photo_url: providerData.photoURL,
      }

      await insertOne('client', newUser)
